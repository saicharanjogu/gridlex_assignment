...
const tourSteps: TourStep[] = [
  ...
  },
  {
    ...
    description: 'Start managing your data with Gridlex. Press Command+K to search or ? for keyboard shortcuts.',
      ...
      <Card className="w-full max-w-md mx-4 shadow-2xl border-primary/20">
        <CardHeader className="relative pb-2">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div 
              className="w-12 h-12 rounded-xl gradient-primary-subtle border border-primary/20 flex items-center justify-center"
              aria-hidden="true"
            >
              {step.icon}
            </div>
            <div>
              ...
              <CardTitle id="onboarding-title" className="text-lg">{step.title}</CardTitle>
            </div>
          </div>
          <Progress 
            value={progress} 
        ...
        <CardContent>
          <p id="onboarding-description" className="text-muted-foreground">{step.description}</p>
        </CardContent>
        <CardFooter className="flex justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button 
              ...
              aria-label={currentStep === tourSteps.length - 1 ? 'Complete tour and get started' : 'Go to next step'}
            >
              {currentStep === tourSteps.length - 1 ? 'Get Started' : 'Next'}
              {currentStep < tourSteps.length - 1 && <ChevronRight className="h-4 w-4 ml-1" aria-hidden="true" />}
            </Button>
          </div>
        </CardFooter>
      </Card>
